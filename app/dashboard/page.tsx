...
//   // ── Auto-polling: refresh messages every 3s when a conversation is open ──
...
//           if (lastNew !== lastOld) return sorted;
//           return prev;
//         });
//       } catch {
...
//       cr89e_crmwhatsappid: `optimistic-${Date.now()}`,
//       cr89e_messagetext: text.trim(),
//       cr89e_direction: 833680001,
//       cr89e_timestamp: new Date().toISOString(),
//       cr89e_name: `${user.firstName} ${user.lastName}`,
//     };
//     if (text.trim()) {
...
//     if (sentText.trim()) formData.append("message", sentText);
...
//         // Remove optimistic message and show auth error
//         setMessages((prev) => prev.filter((m) => !m.cr89e_crmwhatsappid.startsWith("optimistic-")));
//         showToast("⚠️ Session expired — please log out and log back in", "error");
//         return;
//       }
//       setMessages((prev) => prev.filter((m) => !m.cr89e_crmwhatsappid.startsWith("optimistic-")));
//       showToast("❌ Failed to send: " + (errData.error || "Unknown error"), "error");
//       return;
//     }

//     showToast("✅ Message sent!");

//     // ✅ Wait 1.5s before reloading so the CRM has time to persist the message.
//     // Without this delay, the immediate reload fetches stale data and wipes the
//     // optimistic message. The 3s poll will also catch it naturally.
//     setTimeout(() => {
//       if (selectedRef.current?.cr89e_crmconversationid === selected.cr89e_crmconversationid) {
...
//                           // their mime type directly, so detection is always accurate.
...
//                           if (mediaType === "audio") {
//                             return (
//                               <div className={`mt-2 p-2 rounded-2xl ${isIn ? "bg-[#F5F5DC]" : "bg-white/20"}`}>
//                                 <audio src={url} controls className="w-full max-w-xs" preload="metadata" />
//                               </div>
//                             );
//                           }

//                           const fileName = decodeURIComponent(url.split("/").pop()?.split("?")[0] || m.cr89e_filename || "File");
//                           return (
//                             <div className={`mt-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 p-2 sm:p-3 rounded-2xl ${isIn ? "bg-[#F5F5DC]" : "bg-white/20"}`}>
//                               <div className="flex flex-col min-w-0">
//                                 <span className="font-bold text-xs break-all">{fileName}</span>
//                                 <span className="text-xs opacity-60">📎 Attachment</span>
//                               </div>
//                               <a href={url} target="_blank" download={m.cr89e_filename} className={dlBtnClass}>Download</a>
//                             </div>
//                           );
//                         })()}
//                       </div>
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {currentLanguage !== "auto" && (
                      <span className="text-[10px] font-black bg-[#48CAE4]/10 text-[#48CAE4] px-2 py-0.5 rounded-full hidden sm:inline">
                        🌐 {currentLanguage}
                      </span>
                    )}
                  </div>
        ...
        `}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border-2 border-[#48CAE4]/10">
              ...
              <p className="text-[10px] text-[#4A4E69]/40 font-medium mb-3">
                {currentLanguage === "auto" ? "Auto-detecting from customer messages" : `Forced: ${currentLanguage}`}
              </p>
              <select
                value={currentLanguage}
                onChange={(e) => setConvLanguage(e.target.value)}
                className="w-full bg-[#F5F5DC]/50 border-2 border-[#4A4E69]/5 rounded-2xl px-3 py-2.5 text-xs font-bold text-[#4A4E69] outline-none focus:border-[#48CAE4]/40 transition cursor-pointer"
              >
                {LANGUAGES.map((l) => (
